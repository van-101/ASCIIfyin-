...
# # Reading Input Image
# x,frame=vid.read()
image = cv2.imread('/home/rasputina/py_comding/opencv/johnny.jpg')
...
for i in range(num_rows):
    for j in range(num_cols):
        partial_image = image[int(
            i*cell_h):min(int((i+1)*cell_h), height), int(j*cell_w):min(int((j+1)*cell_w), width), :]
